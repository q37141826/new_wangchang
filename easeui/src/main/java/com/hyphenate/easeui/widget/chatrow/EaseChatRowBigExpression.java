    ...
    public void onSetUpView() {
        ...
        if((message.ext().get("em_expression_id")!=null)){
          int  emoRes = EmojiParse.isHaveThis(message.ext().get("em_expression_id").toString());
            Glide.with(activity).load(emoRes).placeholder(R.drawable.ease_default_expression).into(imageView);
        }else {
            int  emoRes = EmojiParse.isHaveThis(message.getBody().toString());
            Glide.with(activity).load(emoRes).placeholder(R.drawable.ease_default_expression).into(imageView);
        }
    }
}
