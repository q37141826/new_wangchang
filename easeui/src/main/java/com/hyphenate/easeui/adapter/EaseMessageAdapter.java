...
import com.hyphenate.easeui.bean.ConstantString;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    Handler handler = new Handler() {
        private void refreshList() {
            ...
            List<EMMessage> newVar = new ArrayList<>();
            if (IS_GROUP) {
                newVar.addAll(var);
                ...
                for (int i = 0; i < var.size(); i++) {
                    try {
                        if ((var.get(i).ext().get(ConstantString.CONSULTING_ID) + "").equals(ConsultingId.getConsultingId()) || (var.get(i).ext().get(ConstantString.CONSULTING_ID) == null)) {
                            newVar.add(var.get(i));
                            var.get(i).isUnread();
                            var.get(i).setUnread(false);
                            var.get(i).setAttribute(ConstantString.IS_READ, "yes");
                        }//todo web段如果OK了就把这个地方的判断打开
                    } catch (Exception e) {
                    }
                }
            }
//            newVar.addAll(var);
            messages = newVar.toArray(new EMMessage[newVar.size()]);
//            conversation.markAllMessagesAsRead();
            notifyDataSetChanged();//todo 这个地方可以对接收到的消息进行处理
        }
    ...
    protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        switch (message.getType()) {
            case TXT:
                if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
                    presenter = new EaseChatBigExpressionPresenter();//大表情，说明发送消息的时候要添加一个这样的扩展属性
                } else {
